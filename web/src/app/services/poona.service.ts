...
export class PoonaService {
  ...
  public getFederation(): Observable<Instance> {
    return this.http.get(environment.services_basepath + '/poona/federation')
      .map((res: any) => this.mapInstance(res));
  }
  
  public getLigues(): Observable<Instance[]> {
    return this.http.get(environment.services_basepath + '/poona/federation/ligue')
      .map((res: any) => this.mapListeInstances(res));
  }
  
  public getCodeps(ligueId: number): Observable<Instance[]> {
    return this.http.get(environment.services_basepath + '/poona/federation/ligue/' + ligueId + "/codep")
      .map((res: any) => this.mapListeInstances(res));
  }
  
  public getClubs(ligueId: number, codepId: number): Observable<Instance[]> {
    return this.http.get(environment.services_basepath + '/poona/federation/ligue/' + ligueId + "/codep/" + codepId + "/club")
      .map((res: any) => this.mapListeInstances(res));
  }
  
  public getClub(clubId: number): Observable<Instance> {
    return this.http.get(environment.services_basepath + '/poona/federation/club/' + clubId)
      .map((res: any) => this.mapInstance(res));
  }
